...
Create an instance with your `clientId`. You can optionally pass `apiBaseUrl`, `authToken`, [env](cci:7://file:///Users/gustavjacobsen/payment-test/.env:0:0-0:0), and other options.
...
**UMD Usage**
```html
<!-- Set runtime base URL BEFORE loading the UMD build -->
<script>
...
env
 (string, optional): Environment string (e.g., sandbox or production)
...
extraHeaders (Record<string, string>, optional): Additional headers to send with requests
...
Runtime Configuration: Use a runtime base URL (via window.__ACOUNTPAY_API_URL__) to avoid rebuilds when switching environments
